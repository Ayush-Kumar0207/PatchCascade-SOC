...
| A runtime pickle from an untrusted contributor was treated like passive data | `cloudpickle` deserialization can execute attacker-controlled code | Runtime sidecars are labelled trusted-run-only; resume requires explicit trust, while verification hashes and inspects metadata without deserializing |
...
| Factorized MultiDiscrete PPO wastes mass on jointly invalid node/CVE pairs, while tuning it before comparing an untuned MaskablePPO could bias the choice | Learning may be unnecessarily handicapped or the baseline could receive an unfair tuning advantage | A locked interface-first stage gives both exact implementations identical reference hyperparameters, training seeds, budgets, and paired validation episodes; safety and per-task bootstrap rules mechanically write the interface decision before the 8→3→2→1 hyperparameter campaign runs on its winner. Synthetic CI is never evidence and no superiority is claimed. |
